...
mid x y z
   | max (max x y) z == x = if max y z == y then y else z
   | max (max x y) z == y = if max x z == x then x else z
   | max (max x y) z == z = if max x y == x then x else y
...
sumLast2 n = 
   let digit0 = abs n `mod` 10
       digit1 = (abs n `mod` 100) `div` 10
   in digit1 + digit0
...
firstDigit n = if 0 <= abs n && abs n <= 9
   then abs n
