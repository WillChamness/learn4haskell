...
In this chapter, you are going to master:
 ...
 ✧ Eta-reduction As usual, the explanations are in the Haskell comments of this
...
subList :: Int -> Int -> [a] -> [a]
subList _ _ [] = []
subList start stop xs = if start < 0 || stop < 0 
   then [] 
   else drop start (take (stop + 1) xs)
...
smartReplicate [] = []
smartReplicate l = replicate (head l) (head l) ++ smartReplicate (tail l)
...
contains :: Eq a => a -> [[a]] -> [[a]]
contains _ [] = []
contains n (lst:rest) = if n `elem` lst
   then lst : contains n rest
   else contains n rest
...
rotate :: Int -> [a] -> [a]
rotate _ [] = []
rotate n lst = slice n (n + length lst) (cycle lst)
   where
      slice :: Int -> Int -> [a] -> [a]
      slice _ _ [] = []
      slice start stop xs 
         | start < 0 || stop < 0 = []
         | start >= stop = []
         | otherwise = drop start (take stop xs)
